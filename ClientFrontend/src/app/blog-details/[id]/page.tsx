...
import blogData from "@/components/Blog/blogData"; // Import your blog data
import Image from "next/image"; // Import Image from Next.js

// Dynamic page to handle blog details
const BlogDetailsPage = ({ params }: { params: { id: string } }) => {
  const { id } = params; // Get the dynamic 'id' from the URL (using params instead of useRouter())

  // Find the blog with the corresponding ID
  const blog: Blog | undefined = blogData.find((blog) => blog.id === parseInt(id));

  if (!blog) {
    return <p>Error: Blog not found</p>; // Handle missing blog
  }
